...
AVL.HIGHLIGHT_COLOR = "#000277" // circulo de gira y hace el balanceo
AVL.HEIGHT_LABEL_COLOR = "#060606" // color de la letra
AVL.LINK_COLOR = "#060606";
AVL.HIGHLIGHT_CIRCLE_COLOR = "#060606";
AVL.FOREGROUND_COLOR = "#556BF5";  //borde de la burbuja
AVL.BACKGROUND_COLOR = "#FFFFFF";  // interior de la burbuja
AVL.PRINT_COLOR = AVL.FOREGROUND_COLOR;
AVL.WIDTH_DELTA  = 60;
AVL.HEIGHT_DELTA = 60;
AVL.STARTING_Y = 60;
AVL.FIRST_PRINT_POS_X  = 60;
AVL.PRINT_VERTICAL_GAP  = 30;
AVL.PRINT_HORIZONTAL_GAP = 60;
AVL.EXPLANITORY_TEXT_X = 20;
AVL.EXPLANITORY_TEXT_Y = 20;
...
AVL.prototype.addControls =  function(){
	...
	this.findButton = addControlToAlgorithmBar("Button", "Encontrar nodo");
	this.findButton.onclick = this.findCallback.bind(this);
	this.printButton = addControlToAlgorithmBar("Button", "Imprimir Nodo");
...
AVL.prototype.insertElement = function(insertedValue){
	this.commands = [];	
	this.cmd("SetText", 0, " Insertando valor "+insertedValue);
...
AVL.prototype.insert = function(elem, tree)
{
	...
	if (elem.data < tree.data){
		this.cmd("SetText", 0, elem.data + " < " + tree.data + ". Buscando el sub-arbol izquierdo");				
	}else{
		this.cmd("SetText",  0, elem.data + " >= " + tree.data + ". Buscando el sub-arbol derecho");				
	}
	this.cmd("Step");
	this.cmd("SetHighlight", tree.graphicID , 0);
	...
	if (elem.data < tree.data){
		if (tree.left == null){
			this.cmd("SetText", 0, "Arbol nullo, Insertando elemento");				
			...
			this.cmd("Move", this.highlightID, tree.x, tree.y);
			this.cmd("SetText",  0, "Recurrencia relajada");			
			this.cmd("Step");
			this.cmd("Delete", this.highlightID);
			...
			if (tree.height < tree.left.height + 1){
				...
				this.cmd("SetText",  0, "Ajustanado la altura despues de la llamada recursiva");			
			...
			if (tree.height < tree.left.height + 1){
				tree.height = tree.left.height + 1
				this.cmd("SetText",tree.heightLabelID,tree.height); 
				this.cmd("SetText",  0, "Ajuste de altura despues de la recursio");			
				this.cmd("SetForegroundColor", tree.heightLabelID, AVL.HIGHLIGHT_LABEL_COLOR);
				this.cmd("Step");
				this.cmd("SetForegroundColor", tree.heightLabelID, AVL.HEIGHT_LABEL_COLOR);
			}
		...
		if (tree.right == null){
			this.cmd("SetText",  0, "Arbol nullo encontrado, insertando elemento");			
			this.cmd("SetText", elem.heightLabelID,1); 
			this.cmd("SetHighlight", elem.graphicID, 0);
			tree.right=elem;
			elem.parent = tree;
			...
			if (tree.height < tree.right.height + 1)
			{
				...
				this.cmd("SetText",tree.heightLabelID,tree.height); 
				this.cmd("SetText",   0, "Ajuste de altura, despues de la llamada recursiva");			
				this.cmd("SetForegroundColor", tree.heightLabelID, AVL.HIGHLIGHT_LABEL_COLOR);
				this.cmd("Step");
				this.cmd("SetForegroundColor", tree.heightLabelID, AVL.HEIGHT_LABEL_COLOR);						
			}
		...
		else
		{
			...
			if (tree.height < tree.right.height + 1)
			{
				tree.height = tree.right.height + 1
				this.cmd("SetText",tree.heightLabelID,tree.height); 
				this.cmd("SetText",  0, "Ajuste de altura despues de la recursion");			
				this.cmd("SetForegroundColor", tree.heightLabelID, AVL.HIGHLIGHT_LABEL_COLOR);
				this.cmd("Step");
				this.cmd("SetForegroundColor", tree.heightLabelID, AVL.HEIGHT_LABEL_COLOR);
...
AVL.prototype.treeDelete = function(tree, valueToDelete)
{
	...
	if (tree != null)
	{
		...
		{	
			this.cmd("SetText", 0, valueToDelete + " < " + tree.data + ".  Buscando subarbol inquierdo");				
		}
		else if (valueToDelete > tree.data)
		{
			this.cmd("SetText", 0, valueToDelete + " > " + tree.data + ". Buscando subarbol derecho");				
		}
		else
		{
			this.cmd("SetText", 0, valueToDelete + " == " + tree.data + ".  Encontrando nodo a eliminar");									
		}
		this.cmd("Step");
		this.cmd("SetHighlight", tree.graphicID, 0);
		...
		if (valueToDelete == tree.data)
		{
			...
			{
				this.cmd("SetText",  0, "Nodo a eliminar es una hoja.  Eliminarlo.");									
